...
cols = c("category","New_taches_Monday", "hr")
...
ui <- fluidPage(
  ...
  # Page title 
  titlePanel("Habitat - hR: sommaire heures clockify et tâches Monday"),
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Section : importation 
      fileInput(inputId = "upload", # Will be used in the server 
      ...
      numericInput(inputId = "price", label = "Prix ($) :", value = 0),
      
      # Section : exportation 
      tags$h4("Paramètres d'exportation"), # Apply an HTML tag to text 
      ## Selection of Export type 
      radioButtons("file_type", "Sélectionner le format d'exportation:",
                   choices = c("CSV" = ".csv", 
                               "Excel" = ".xlsx"
                   )
      ),
  ...
  output$download_data <- downloadHandler(
    ...
    content = function(file) {
      if (input$file_type == ".csv") {
        readr::write_excel_csv(x = summary_df(), file = file)
      } else {
